...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  prev_h = h0
  for i in range(T):
    h_t, cache_t = rnn_step_forward(x[:,i,:], prev_h, Wx, Wh, b)
    prev_h = h_t
    h[:,i,:] = h_t
    cache += cache_t
  ##############################################################################
...
def word_embedding_forward(x, W):
  ...
  N,T = x.shape
  V,D = W.shape
  out = np.array([[W[j]] for i in x for j in i]).reshape((N,T,D))
  cache = x, W
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_backward(dh, cache):
  ...
  for t in range(T-1, -1, -1):
    dx_t, d_prev_ht, d_prev_ct, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:,t,:] + d_prev_ht, d_prev_ct, cache[t])
    if(np.ndim(dx) < 3):
      dx.resize((N, T, dx_t.shape[1]))
    dx[:,t,:] = dx_t
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
